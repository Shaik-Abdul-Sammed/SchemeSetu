  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }} className="container py-8">
      <h1 style={{ fontSize: '1.75rem', color: '#0B192C', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
        <Settings style={{ color: '#0284C7' }} size={28} /> Administrative Dynamic Scheme Portal
      </h1>
      <p style={{ color: '#64748B', marginBottom: '2rem' }}>
        Government Officials Panel: Dynamically create, edit, or remove schemes and loan parameters.
      </p>

      {/* Add Scheme Form */}
      <form onSubmit={handleAdd} className="card" style={{ marginBottom: '2rem' }}>
        <h3 style={{ fontSize: '1.1rem', color: '#0F172A', marginBottom: '1rem' }}>+ Create New Scheme Entry</h3>
        ...
        <button type="submit" className="btn btn-green">
          <Plus size={16} /> Publish Scheme
        </button>
      </form>

      {/* Active Schemes Table */}
      <div className="card">
        <h3 style={{ fontSize: '1.2rem', color: '#0B192C', marginBottom: '1rem' }}>Active Schemes ({schemes.length})</h3>
          ...
          {schemes.map((s) => (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', background: '#F8FAFC', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
              <div>
                ...
                <div style={{ fontSize: '0.82rem', color: '#64748B' }}>Max Loan: ₹{(s.maxLoan || 300000).toLocaleString('en-IN')}</div>
              </div>
