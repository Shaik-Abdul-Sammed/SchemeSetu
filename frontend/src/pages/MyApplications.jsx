...
import { FileCheck, Calendar, Clock, CheckCircle2, AlertCircle, ArrowRight, Download, Search } from 'lucide-react';
      ...
      const demoApps = [
        {
          ...
          loanAmount: 300000,
          remarks: 'Verification complete. First installment ₹2,000 dispatched to bank account.'
        },
        {
          id: 'APP-2026-9042',
          schemeId: 'pmmy-kishore',
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto' }} className="container py-8">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', color: '#0B192C', margin: 0, display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
            <FileCheck style={{ color: '#059669' }} size={28} /> My Scheme Applications
          </h1>
          <p style={{ color: '#64748B', fontSize: '0.95rem', margin: 0 }}>
            Track real-time status of your saved and submitted government scheme applications.
          </p>
        </div>
      ...
      {applications.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
          ...
          <h3 style={{ fontSize: '1.2rem', color: '#0F172A', marginBottom: '0.5rem' }}>No Saved Applications Yet</h3>
          <p style={{ color: '#64748B', marginBottom: '1.5rem' }}>Find schemes you are eligible for and click "Save & Track Application".</p>
          <Link to="/input" className="btn btn-primary">Start Scheme Search</Link>
        </div>
      ) : (
            ...
            <div key={app.id} className="card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: '0.85rem', color: '#64748B', display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '1rem' }}>
                  <Calendar size={14} /> Submitted: {app.date}
                </div>
                ...
                {app.remarks && (
                  <div style={{ fontSize: '0.85rem', color: '#334155', background: '#F8FAFC', padding: '0.65rem', borderRadius: '6px', border: '1px solid #E2E8F0', marginBottom: '1rem' }}>
                    <strong>Note:</strong> {app.remarks}
                  </div>
                )}
              </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(11,25,44,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '1rem' }}>
          <div className="card glass-card" style={{ maxWidth: '540px', width: '100%', padding: '1.75rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.9rem', color: '#334155', marginBottom: '1.5rem' }}>
              <div><strong>Application Status:</strong> <span style={{ color: selectedApp.status === 'Approved' ? '#059669' : '#D97706', fontWeight: 700 }}>{selectedApp.status}</span></div>
              <div><strong>Submission Date:</strong> {selectedApp.date}</div>
              <div><strong>Requested Loan Amount:</strong> ₹{(selectedApp.loanAmount || 300000).toLocaleString('en-IN')}</div>
              <div><strong>Verification Status:</strong> Document checklist verified & passed.</div>
            </div>

            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setSelectedApp(null)} className="btn btn-secondary btn-sm">Close</button>
              <button onClick={() => handleDownloadPdf(selectedApp)} className="btn btn-primary btn-sm"><Download size={16} /> Download Application PDF</button>
            </div>
          </div>
        </div>
      )}
